	...
	methods: {
		...
		async _deleteDiscussion() {
			...
			if (response.status == 200) {
				...
				this.$root.routeTo('/a/discussions');
				this.$root.toast(
					'',
					response.message,
					'success'
		...
		async unflagComment(comment_id) {
			let fetch_bearer_token = this.$cookies.get('bearer_token');
...
<template>
				...
				<div class="card mt20">
					...
					<div class="card-body">
						...
						>
							...
							<p class="mt5">
								<span class="bold fs18 pointer underline" @click="this.$root.routeTo(`/a/ballot/${discussion.associated_ballot}`)">
									{{ discussion.ballot_title }}
								</span>
							</p>
						</div>
						...
						<div class="comments-scoll-section mt80 pl5">
	...
	>
		<div class="modal-container">
			...
			<p class="pt15 pb15">Lock/archive this discussion? The discussion will no longer be able to be modified, deleted, or commented on once this action is done. People will still be able to like/pin the post.</p>
